...
try:
    ...
    # Optional bundle support (dramatically reduces UDP packet rate to Max).
    try:
        from pythonosc.osc_bundle_builder import OscBundleBuilder
        from pythonosc.osc_bundle_builder import IMMEDIATELY as OSC_IMMEDIATELY
        from pythonosc.osc_message_builder import OscMessageBuilder
    except Exception:
        OscBundleBuilder = None
        OscMessageBuilder = None
        OSC_IMMEDIATELY = None
    OSC_AVAILABLE = True
except ImportError:
    OSC_AVAILABLE = False
...
class OscTrackSender:
    """
    Dedicated sender thread for Max/tracker OSC (port 9001 by default).
    Goals:
    - Keep sensing/UI threads responsive (no UDP send in hot loops).
    - Keep memory bounded (bounded queue, drop-old).
    - Reduce packet rate (bundle multiple messages into one UDP packet when possible).
    """

    def __init__(self, client: Optional[object], *, queue_max: int = 4):
        self._client = client
        self._queue: deque[list[tuple[str, object]]] = deque(maxlen=max(1, int(queue_max)))
    ...
    def send_bundle(self, items: list[tuple[str, object]]) -> None:
        """
        Enqueue a bundle (list of (address, payload)).
        If queue is full, we drop the oldest bundle automatically (deque(maxlen=...)).
        """
    ...
    @staticmethod
    def _add_payload(mb, payload: object) -> None:
        # python-osc expects add_arg per argument (Max expects args, not a single list object).
    ...
    def _worker(self) -> None:
        while self._running:
            ...
            # IMPORTANT: Max side in this project does NOT parse OSC bundles reliably.
            # So we *always* send individual OSC messages (still off the UI thread).
            for addr, payload in items:
                try:
                    client.send_message(str(addr), payload)
                except Exception:
                    pass
...
class AudioViewerWidget(QWidget):
    ...
    def __init__(self, width: int, height: int):
        ...
        self.osc_track_sender: Optional[OscTrackSender] = None
        ...
        # A second, much slower-evolving system state for Max (/system/stateB).
        # Same target as /system/state, but with a larger smoothing time-constant to avoid jumps.
        self.osc_tracker_systemB_enabled: bool = True
        self.osc_tracker_systemB_rate_hz: float = 5.0
        self.osc_tracker_systemB_min_delta: float = 0.01
        self.osc_tracker_systemB_tau_s: float = 8.0
        ...
        self._osc_last_send_systemB_monotonic: float = 0.0
        ...
        self._osc_last_sent_systemB_state: Optional[float] = None
        ...
        self._osc_stateB_value: float = 1.0
        ...
        self.set_state_enabled: bool = True
        self.set_state_count: int = 7
        self.set_min_hold_s: float = 18.0  # do not change sets faster than this
        # Planned schedule for current track: list of (t0, t1, set_n)
        self._set_schedule: list[tuple[float, float, int]] = []
        self._last_sent_set: Optional[int] = None
        self._last_set_send_monotonic: float = 0.0

        # Cross-track memory (persisted on disk)
        self._set_memory_path = Path(__file__).resolve().parent / "set_state_memory.json"
        self._set_usage_counts: list[int] = [0 for _ in range(self.set_state_count)]
        self._set_last_used: Optional[int] = None
        self._load_set_memory()
    ...
    def _load_audio_file(self, audio_path: Path):
        ...
        try:
            ...
            self._plan_set_schedule_for_current_track(audio_path)
            self._cache_dirty = True
    ...
    # ----------------------- Set-state planning + memory -----------------------

    def _load_set_memory(self) -> None:
        try:
            if not self._set_memory_path.exists():
                return
            with open(self._set_memory_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            counts = data.get("usage_counts")
            last_used = data.get("last_used")
            if isinstance(counts, list) and counts:
                # Normalize to current set_state_count
                counts_i = []
                for v in counts[: self.set_state_count]:
                    try:
                        counts_i.append(int(v))
                    except Exception:
                        counts_i.append(0)
                if len(counts_i) < self.set_state_count:
                    counts_i.extend([0] * (self.set_state_count - len(counts_i)))
                self._set_usage_counts = counts_i
            if last_used is not None:
                try:
                    lu = int(last_used)
                    if 1 <= lu <= self.set_state_count:
                        self._set_last_used = lu
                except Exception:
                    pass
        except Exception:
            # Never crash the GUI because of a bad memory file.
            return

    def _save_set_memory(self) -> None:
        try:
            data = {
                "usage_counts": [int(v) for v in self._set_usage_counts],
                "last_used": int(self._set_last_used) if self._set_last_used is not None else None,
            }
            with open(self._set_memory_path, "w", encoding="utf-8") as f:
                json.dump(data, f, indent=2)
        except Exception:
            return

    def _choose_set_segment_count(self, duration_s: float) -> int:
        """
        Decide how many set changes we want inside one track.
        Short tracks -> 1 set. Longer tracks -> 3-4 sets.
        """
    ...
    def _dominant_pattern_in_range(self, t0: float, t1: float) -> int:
        """
        Return the dominant pattern id (1..pattern_count) inside [t0, t1).
        Falls back to label_at(t0) when we have no frames.
        """
        if not self.features.is_valid:
            return 1
        try:
            t = self.features.t
            if t.size == 0 or self.features.cluster_labels.size == 0:
                return int(self.features.label_at(t0))
            m = (t >= float(t0)) & (t < float(t1))
            if not np.any(m):
                return int(self.features.label_at(t0))
            # Map cluster ids to pattern ids (1..pattern_count)
            cids = self.features.cluster_labels[m].astype(np.int32)
            pats = np.array([int(self.features.cluster_to_pattern.get(int(cid), 1)) for cid in cids], dtype=np.int32)
            pats = np.clip(pats, 1, int(self.pattern_count))
            counts = np.bincount(pats, minlength=int(self.pattern_count) + 1)
            # ignore 0 bin
    ...
    def _available_set_count_hint(self) -> int:
        """
        Best-effort hint: if we can read pattern_sets.json locally, cap set sending
        to the number of sets on disk. Otherwise, fall back to set_state_count.
        """
        try:
            p = Path(__file__).resolve().parent / "pattern_sets.json"
            if not p.exists():
                return int(self.set_state_count)
            with open(p, "r", encoding="utf-8") as f:
                data = json.load(f)
            sets = data.get("sets", [])
            if isinstance(sets, list) and len(sets) > 0:
                return int(max(1, min(self.set_state_count, len(sets))))
        except Exception:
            pass
        return int(self.set_state_count)

    def _pick_set_with_memory(self, preferred: int, *, avoid: Optional[int]) -> int:
        """
        Choose a set (1..set_state_count) biased toward `preferred`, but also:
        - avoid repeating `avoid` when possible
        - prefer underused sets globally (cross-track memory)
        """
        preferred = int(np.clip(int(preferred), 1, self.set_state_count))
        avoid_n = int(avoid) if avoid is not None else None

        max_used = max(self._set_usage_counts) if self._set_usage_counts else 0

        best_score = None
        best_candidates: list[int] = []
        for s in range(1, self.set_state_count + 1):
            used = self._set_usage_counts[s - 1] if (s - 1) < len(self._set_usage_counts) else 0
            score = 0.0
            # preference for dominant state
            if s == preferred:
                score += 4.0
            # global "coverage" encouragement
            score += float(max_used - used) * 0.35
            # penalize immediate repeats
            if avoid_n is not None and s == avoid_n:
                score -= 3.0
            # slight penalty for repeating last-used across tracks
            if self._set_last_used is not None and s == self._set_last_used:
                score -= 1.0

            if best_score is None or score > float(best_score) + 1e-9:
                best_score = score
                best_candidates = [s]
            elif abs(score - float(best_score)) <= 1e-9:
                best_candidates.append(s)

        if not best_candidates:
            return preferred

        # Deterministic-ish tie break per track, but still varies across different audio files.
        try:
            seed = 0
            if self.current_file is not None:
                h = hashlib.md5(str(self.current_file).encode("utf-8")).hexdigest()
                seed = int(h[:8], 16)
            rng = random.Random(seed)
            return int(rng.choice(best_candidates))
        except Exception:
            return int(random.choice(best_candidates))

    def _plan_set_schedule_for_current_track(self, audio_path: Path) -> None:
        """
        Plan a slow set schedule for the current track. This is computed once per file load.
        """
        self._set_schedule = []
        self._last_sent_set = None
        self._last_set_send_monotonic = 0.0

        if not self.set_state_enabled or not self.features.is_valid:
            return

        duration_s = float(self.features.duration or 0.0)
        if duration_s <= 0.0:
            return

        # Cap to how many sets exist on disk (if available).
        set_count_hint = self._available_set_count_hint()
        if set_count_hint <= 0:
            return

        k = int(np.clip(self._choose_set_segment_count(duration_s), 1, 7))
        seg_len = duration_s / float(k)

        prev_set: Optional[int] = None
        for i in range(k):
            t0 = float(i * seg_len)
            t1 = float(duration_s if i == (k - 1) else (i + 1) * seg_len)
            dom_pat = int(self._dominant_pattern_in_range(t0, t1))
            raw_set = self._pick_set_with_memory(dom_pat, avoid=prev_set)
            # If controller has fewer sets than 7, wrap safely.
            set_n = 1 + ((int(raw_set) - 1) % int(set_count_hint))
            self._set_schedule.append((t0, t1, int(set_n)))
            prev_set = int(set_n)

        # If segments collapsed to the same set, keep it simple (still fine for "short audio" intent).
        # Otherwise, this will naturally produce 2-4 set changes on long tracks.
        if self.osc_log_cb.isChecked():
            print(f"[AUDIO/SET] planned k={k} dur={duration_s:0.2f}s schedule={self._set_schedule}")
    ...
    def set_osc(
        ...
        tracker_systemB_enabled: bool = True,
        tracker_systemB_rate_hz: float = 5.0,
        tracker_systemB_min_delta: float = 0.01,
        tracker_systemB_tau_s: float = 8.0,
        ...
        # Track target (Max) can be either a direct python-osc client OR a dedicated sender thread.
        if isinstance(client_track, OscTrackSender):
            self.osc_track_sender = client_track
            self.osc_client_track = None
        else:
            self.osc_track_sender = None
            self.osc_client_track = client_track

        # Enable OSC if the user wants it AND we have at least one destination (lights OR Max).
        # IMPORTANT: the Max tracker stream must be able to run even if the lights controller is off.
        has_pattern = self.osc_client_pattern is not None
        has_track = (self.osc_track_sender is not None) or (self.osc_client_track is not None)
        self.osc_enabled = bool(enabled) and (has_pattern or has_track)
        ...
        self.osc_tracker_systemB_enabled = bool(tracker_systemB_enabled)
        self.osc_tracker_systemB_rate_hz = float(tracker_systemB_rate_hz)
        self.osc_tracker_systemB_min_delta = float(max(0.0, tracker_systemB_min_delta))
        self.osc_tracker_systemB_tau_s = float(max(0.001, tracker_systemB_tau_s))
        ...
        if self.osc_enabled:
            ...
            track_sender = type(self.osc_track_sender).__name__ if self.osc_track_sender is not None else "None"
            pat_name = type(client_pattern).__name__ if client_pattern is not None else "None"
            print(f"[AUDIO/OSC] enabled pattern_client={pat_name} track_client={track_name} track_sender={track_sender} format={self.osc_format} send_features={self.osc_send_features} rate_hz={self.osc_send_rate_hz} pattern_max={self.pattern_max} pattern_count={self.pattern_count}")
        else:
            if enabled:
    ...
    def _osc_send(self, address: str, value, *, target: str = "pattern"):
        if not self.osc_enabled:
            return
        if target == "track":
            if self.osc_track_sender is not None:
                self.osc_track_sender.send_bundle([(str(address), value)])
                return
    ...
    def _tick_playback(self):
        ...
        # Lights target (pattern controller) is optional: if it's not configured/running,
        ...
        if self.osc_client_pattern is not None:
            ...
            # Slow meta-layer: send /set N only when entering a planned segment (rate-limited).
            self._maybe_send_set_for_playhead()
    ...
    def _maybe_send_set_for_playhead(self) -> None:
        if not self.set_state_enabled:
            return
        if not self.osc_enabled or self.osc_client_pattern is None:
            return
        if not self._set_schedule:
            return
        if not self.features.is_valid:
            return

        # Find current segment set
        desired: Optional[int] = None
        t = float(self.playhead_t)
        for t0, t1, s in self._set_schedule:
            if t >= float(t0) and t < float(t1):
                desired = int(s)
                break
        if desired is None:
            return

        if self._last_sent_set is not None and int(desired) == int(self._last_sent_set):
            return

        now = time.monotonic()
        if (now - float(self._last_set_send_monotonic)) < float(self.set_min_hold_s):
            return

        self._last_sent_set = int(desired)
        self._last_set_send_monotonic = float(now)
        self._set_last_used = int(desired)
        # Increment usage counts for coverage memory (1..set_state_count)
        if 1 <= int(desired) <= self.set_state_count:
            self._set_usage_counts[int(desired) - 1] = int(self._set_usage_counts[int(desired) - 1]) + 1
        self._save_set_memory()

        # Send to light controller (pattern target) as a separate message.
        self._osc_send("/set", int(desired), target="pattern")
...
class GraphicScoreWidget(QLabel):
    ...
    def update_score_from_frame(self, adjusted_inverted: np.ndarray, detections: Optional[list] = None):
        ...
        if self.style == "spectrogram":
            ...
            # Vectorized lines (avoid per-pixel Python loops, which can freeze the UI)
            for y in range(0, self.score_height, self.grid_spacing_y):
                row = display[y, :, 0]
                m = row < 50
                if np.any(m):
                    display[y, m, :] = self.grid_color

            for x in range(0, self.score_width, self.grid_spacing_x):
                col = display[:, x, 0]
                m = col < 50
                if np.any(m):
                    display[m, x, :] = self.grid_color
        
        # Subtle cursor line (on left where new content appears)
        cursor_x = stamp_width + 1
        if cursor_x < self.score_width:
            col = display[:, cursor_x, 0]
            m = col < 100
            if np.any(m):
                display[m, cursor_x, :] = (0, 80, 0)
    ...
    def display_cv_image(self, cv_image: np.ndarray):
        ...
        h, w, ch = rgb_image.shape
        bytes_per_line = ch * w
        # IMPORTANT: copy() so the QImage owns the memory. Without this, QImage can reference
        # a temporary numpy buffer that gets freed, leading to corruption / freezes / crashes.
        qt_image = QImage(rgb_image.data, w, h, bytes_per_line, QImage.Format_RGB888).copy()
        self.setPixmap(QPixmap.fromImage(qt_image))


class GraphicScoreState:
    """
    Thread-safe (non-Qt) score renderer state.
    This is a functional copy of the rendering logic from GraphicScoreWidget, but it does NOT touch Qt.
    The worker thread owns this state and returns a rendered BGR image for the UI to display.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.score_width = int(width)
        self.score_height = int(height)

        # Canvas for spectrogram / score (BGR uint8)
        self.canvas = np.zeros((self.score_height, self.score_width, 3), dtype=np.uint8)
        self.scroll_position = 0
        ...
        self.core_thickness = 1
        self.halo_enabled = bool(halo_enabled)
        self.halo_kernel = int(halo_kernel)
        ...
        self.spectro_noise_floor = int(spectro_noise_floor)
        self.spectro_grain = float(spectro_grain)
        self.spectro_blur_y = int(spectro_blur_y)
        self.spectro_log_k = float(spectro_log_k)
        self.spectro_background_level = int(spectro_background_level)
        self.spectro_ink_strength = int(spectro_ink_strength)
        self.spectro_speckle_strength = int(spectro_speckle_strength)
        self.spectro_mask_feather = int(spectro_mask_feather)
        self.spectro_intensity_weight = float(spectro_intensity_weight)
        self.spectro_motion_weight = float(spectro_motion_weight)
        self.spectro_profile_smooth = float(spectro_profile_smooth)
        self.spectro_profile_mode = str(spectro_profile_mode).strip().lower()
        if self.spectro_profile_mode not in ("max", "mean"):
            self.spectro_profile_mode = "max"
        ...
        self._prev_img_scaled: Optional[np.ndarray] = None
        self._profile_ema: Optional[np.ndarray] = None

        # Grid settings
        self.grid_spacing_x = 50
        self.grid_spacing_y = 40
        self.grid_color = (25, 25, 25)

        # Timeline marker settings
        self.timeline_interval = 30
        self.timeline_y = 8
    ...
    def update_score_from_frame(self, adjusted_inverted: np.ndarray, detections: Optional[list] = None) -> np.ndarray:
        # Keep logic identical to widget version; just return the BGR display image.
        self._apply_decay()
        ...
        source_h, source_w = light_mask.shape[:2]
        scale_factor = self.score_height / max(1, source_h)
        scaled_w = int(source_w * scale_factor)
        scaled_w = max(1, scaled_w)

        mask_scaled = cv2.resize(light_mask, (scaled_w, self.score_height), interpolation=cv2.INTER_NEAREST)
        img_scaled = cv2.resize(adjusted_inverted, (scaled_w, self.score_height), interpolation=cv2.INTER_LINEAR)
        ...
        if self.style == "spectrogram":
            ...
            if self.spectro_render_mode == "slice":
                ...
                self._render_spectrogram_energy_column(dst_x=dst_x, col_energy=self._profile_ema)
        else:
            sample_x = (self.scroll_position * stamp_width) % scaled_w
            src_x = int(sample_x % scaled_w)
            col_mask = mask_scaled[:, src_x] > 0
...
class CvScoreWorker(QObject):
    """
    Worker that owns camera capture + CV detection + score rendering.
    It emits ready-to-display numpy images back to the UI thread.

    Design goals:
    - Never block the Qt GUI thread.
    - Keep-only-latest behavior: if processing overruns, we drop frames (no backlog).
    - Auto-reconnect camera on repeated read failures.
    """
    ...
    def __init__(self, *, args, panel_width: int, cv_display_height: int, score_height: int, osc_track_sender: Optional[OscTrackSender] = None):
        ...
        self.cv_display_height = int(cv_display_height)
        self.score_height = int(score_height)
        ...
        # Camera transform (mounting compensation)
        self._cam_rotate_deg: int = int(getattr(args, "camera_rotate", 0) or 0)
        self._cam_flip: str = str(getattr(args, "camera_flip", "none") or "none").strip().lower()
        if self._cam_rotate_deg not in (0, 90, 180, 270):
            self._cam_rotate_deg = 0
        if self._cam_flip not in ("none", "h", "v", "hv"):
            self._cam_flip = "none"

        # Optional: ignore humans (mask detections that overlap "person" regions)
        self._ignore_people: bool = bool(getattr(args, "cv_ignore_people", False))
        self._people_rate_hz: float = float(max(0.1, getattr(args, "cv_people_rate_hz", 2.0)))
        self._people_pad_px: int = int(max(0, getattr(args, "cv_people_pad_px", 18)))
        self._people_boxes: list[tuple[int, int, int, int]] = []
        self._people_last_monotonic: float = 0.0
        self._hog = None
        try:
            if self._ignore_people:
        ...
        # CV parameters (controlled from UI)
        self._contrast: float = 1.0
        self._brightness: float = 0.0
        self._gamma: float = 1.0
        self._gamma_lut: Optional[np.ndarray] = None
        self._gamma_lut_gamma: Optional[float] = None

        # Background learning
        self._frame_count: int = 0
        self._bg_learning_frames: int = int(getattr(args, "bg_learning", 120))
        self._learning_phase: bool = True

        # Analyzer exists only for bg subtractor (learning), kept for parity
        self.geometry_analyzer = LightGeometryAnalyzer(history_size=30)

        # Score renderer state
        self.score_state = GraphicScoreState(
            int(getattr(args, "window_width", 1920)),
            int(score_height),
            decay=float(getattr(args, "score_decay", 1.0)),
            draw_strokes=bool(getattr(args, "score_strokes", False)),
            halo_enabled=bool(getattr(args, "score_halo", False)),
            halo_kernel=int(getattr(args, "score_halo_kernel", 9)),
            halo_strength=float(getattr(args, "score_halo_strength", 0.65)),
            stamp_brightness_min=int(getattr(args, "score_stamp_min_brightness", 150)),
            style=str(getattr(args, "score_style", "dots")),
            spectro_noise_floor=int(getattr(args, "score_spectro_noise_floor", 6)),
            spectro_grain=float(getattr(args, "score_spectro_grain", 0.18)),
            spectro_blur_y=int(getattr(args, "score_spectro_blur_y", 3)),
            spectro_log_k=float(getattr(args, "score_spectro_log_k", 12.0)),
            spectro_background_level=int(getattr(args, "score_spectro_background_level", 235)),
            spectro_ink_strength=int(getattr(args, "score_spectro_ink_strength", 210)),
            spectro_speckle_strength=int(getattr(args, "score_spectro_speckle_strength", 22)),
            spectro_mask_feather=int(getattr(args, "score_spectro_mask_feather", 5)),
            spectro_intensity_weight=float(getattr(args, "score_spectro_intensity_weight", 1.0)),
            spectro_motion_weight=float(getattr(args, "score_spectro_motion_weight", 0.7)),
            spectro_profile_smooth=float(getattr(args, "score_spectro_profile_smooth", 0.35)),
            spectro_profile_mode=str(getattr(args, "score_spectro_profile_mode", "max")),
            spectro_render_mode=str(getattr(args, "score_spectro_render_mode", "profile")),
            spectro_slices_per_frame=int(getattr(args, "score_spectro_slices_per_frame", 1)),
        ...
        # If no sender is provided, fall back to direct python-osc client.
        try:
            if self.osc_track_sender is None and OSC_AVAILABLE and bool(getattr(args, "osc_out_enable", False)) and str(getattr(args, "osc_out_format", "pattern")).strip().lower() == "tracker":
    ...
    def set_gamma(self, v: float) -> None:
        g = float(np.clip(float(v), 0.01, 10.0))
        self._gamma = g
        if abs(g - 1.0) < 1e-6:
            self._gamma_lut = None
            self._gamma_lut_gamma = g
            return
        inv_gamma = 1.0 / g
        self._gamma_lut = (np.array([((i / 255.0) ** inv_gamma) * 255 for i in range(256)])
                           .clip(0, 255)
                           .astype(np.uint8))
        self._gamma_lut_gamma = g
    ...
    def _open_camera(self) -> None:
        try:
            self._cap = cv2.VideoCapture(int(getattr(self.args, "camera_index", 0)), cv2.CAP_ANY)
            if self._cap is None or not self._cap.isOpened():
                self._cap = None
                self.status.emit(f"[CAM] open failed (index={int(getattr(self.args, 'camera_index', 0))})")
                return
            ...
            self.status.emit("[CAM] opened")
        except Exception as exc:
            self._cap = None
            self.status.emit(f"[CAM] open exception: {exc}")
    ...
    def _apply_image_adjustments(self, gray_image: np.ndarray) -> np.ndarray:
        img_u8 = cv2.convertScaleAbs(gray_image, alpha=float(self._contrast), beta=float(self._brightness))
        if abs(float(self._gamma) - 1.0) < 1e-6:
            return img_u8
        lut = self._gamma_lut
        if lut is None:
            self.set_gamma(float(self._gamma))
            lut = self._gamma_lut
        if lut is None:
            return img_u8
        return cv2.LUT(img_u8, lut)

    def _apply_camera_transform(self, frame_bgr: np.ndarray) -> np.ndarray:
        """
        Apply mount compensation so the displayed/processed frame is oriented correctly.
        rotate: 0/90/180/270 degrees clockwise
        flip: none | h | v | hv
        """
        out = frame_bgr
        try:
            if self._cam_rotate_deg == 90:
                out = cv2.rotate(out, cv2.ROTATE_90_CLOCKWISE)
            elif self._cam_rotate_deg == 180:
                out = cv2.rotate(out, cv2.ROTATE_180)
            elif self._cam_rotate_deg == 270:
        ...
        try:
            if self._cam_flip == "h":
                out = cv2.flip(out, 1)
            elif self._cam_flip == "v":
                out = cv2.flip(out, 0)
            elif self._cam_flip == "hv":
                out = cv2.flip(out, -1)
        except Exception:
            pass
        return out

    def _update_people_boxes(self, frame_bgr: np.ndarray) -> None:
        """
        Update cached person bounding boxes using OpenCV's built-in HOG people detector.
        This runs at a low rate and on a downscaled image to keep CPU reasonable.
        Boxes are stored in the current frame coordinate system.
        """
        if not self._ignore_people or self._hog is None:
            self._people_boxes = []
            return
        now = time.monotonic()
        min_dt = 1.0 / float(max(0.1, self._people_rate_hz))
        if (now - float(self._people_last_monotonic)) < min_dt:
            return

        self._people_last_monotonic = float(now)
        try:
            ...
            # Downscale for speed (HOG is heavy). Keep aspect ratio.
            target_w = 640
            scale = 1.0
            if w0 > target_w:
            ...
            pad = int(self._people_pad_px)
            inv = 1.0 / float(scale) if scale > 1e-6 else 1.0
            for (x, y, w, h) in rects:
                x0 = int(round(float(x) * inv)) - pad
                y0 = int(round(float(y) * inv)) - pad
                x1 = int(round(float(x + w) * inv)) + pad
                y1 = int(round(float(y + h) * inv)) + pad
                x0 = max(0, x0)
                y0 = max(0, y0)
                x1 = min(w0, x1)
                y1 = min(h0, y1)
            ...
            self._people_boxes = boxes
        except Exception:
            # Fail closed: don't block detections if detector errors out
            self._people_boxes = []

    def _is_in_people_region(self, cx: int, cy: int) -> bool:
        if not self._people_boxes:
            return False
        for x0, y0, x1, y1 in self._people_boxes:
            if cx >= x0 and cx < x1 and cy >= y0 and cy < y1:
                return True
        return False

    def _detect_all_bright_regions(self, frame_bgr: np.ndarray, inverted_gray: np.ndarray) -> dict:
        gray_original = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2GRAY)
        adjusted_original = self._apply_image_adjustments(gray_original)
        ...
        for cnt in contours:
            ...
            # Optional: ignore detections that lie on people
            if self._is_in_people_region(int(cx), int(cy)):
                continue
            detections.append({
                "bbox": (x, y, w, h),
                "center": (cx, cy),
    ...
    def _maybe_send_cv_tracks(self, detections: list, frame_w: int, frame_h: int):
        if self.osc_track_sender is None and self.osc_track_client is None:
            return
        ...
        bundle_items: list[tuple[str, object]] = []
        for i in range(track_n):
            ...
            cur = (float(payload[0]), float(payload[1]), float(payload[2]), float(payload[3]), float(payload[4]),
                   float(payload[5]), float(payload[6]), float(payload[7]))
            dsum = abs(cur[1] - last[1]) + abs(cur[2] - last[2]) + abs(cur[3] - last[3]) + abs(cur[4] - last[4]) + \
                abs(cur[5] - last[5]) + abs(cur[6] - last[6]) + abs(cur[7] - last[7])
            if dsum < min_delta:
                continue
            self._track_last_sent[i] = cur

            bundle_items.append((f"/track/{letter}", payload))

        if not bundle_items:
            return
        if self.osc_track_sender is not None:
            self.osc_track_sender.send_bundle(bundle_items)
            return
        try:
            for addr, payload in bundle_items:
                self.osc_track_client.send_message(str(addr), payload)
        except Exception:
            pass
    ...
    def start(self) -> None:
        ...
        # Drive the loop using a QTimer in THIS thread (requires QThread event loop)
        self._timer = QTimer()
        self._timer.setInterval(int(1000 / float(getattr(self.args, "fps", 30.0))))
    ...
    def _maybe_reconnect(self, reason: str) -> None:
        ...
        self.status.emit(f"[CAM] reconnect: {reason}")
        self._close_camera()
        self._open_camera()
        ...
        try:
            ...
            if not ret or frame is None:
                ...
                if self._cam_fail_count >= 8:
                    self._maybe_reconnect(f"read failed x{self._cam_fail_count}")
                return

            self._cam_fail_count = 0
            frame = self._apply_camera_transform(frame)
            frame = cv2.resize(frame, (self.panel_width, self.cv_display_height), interpolation=cv2.INTER_AREA)
            # Update person detector cache (optional)
            self._update_people_boxes(frame)

            self._frame_count += 1
            if self._learning_phase and self._frame_count <= self._bg_learning_frames:
                _ = self.geometry_analyzer.bg_subtractor.apply(frame, learningRate=0.01)
                progress = int((self._frame_count / max(1, self._bg_learning_frames)) * 100)
                learning_vis = np.zeros((self.cv_display_height, self.panel_width, 3), dtype=np.uint8)
                overlays = [
                    ("Learning background...", 20, self.cv_display_height // 2 - 10, (230, 230, 230), 18),
                    (f"{progress}%", 20, self.cv_display_height // 2 + 20, (200, 200, 200), 18),
                ]
                self.frame_ready.emit(learning_vis, overlays, None)
                return

            if self._learning_phase:
                self._learning_phase = False
                self.status.emit("[WORKER] background learning complete")
            ...
            self.frame_ready.emit(cv_vis, overlays, score_vis)
        finally:
            self._busy = False
...
class IMOLGraphicScoreWindow(QMainWindow):
    ...
    # UI -> worker parameter updates (queued connections across threads)
    sig_contrast = Signal(float)
    sig_brightness = Signal(float)
    sig_gamma = Signal(float)
    ...
    def __init__(self, args):
        ...
        # CV/score processing is handled in a worker thread.
        self._cv_thread: Optional[QThread] = None
        self._cv_worker: Optional[CvScoreWorker] = None
        ...
        # Create a dedicated sender thread for Max/tracker (prevents UI stalls, reduces packets via bundling)
        if self.osc_track_client is not None:
            self.osc_track_sender = OscTrackSender(self.osc_track_client, queue_max=4)

        # Build UI
        self.build_ui()

        # Start CV + score worker (camera + processing off the GUI thread)
        self._start_worker()

        # Initialize worker params to current UI defaults
        self.sig_contrast.emit(float(self.contrast))
        self.sig_brightness.emit(float(self.brightness))
        self.sig_gamma.emit(float(self.gamma))
    ...
    def build_ui(self):
        ...
        self.audio_viewer.set_osc(
            ...
            client_track=(self.osc_track_sender or self.osc_track_client),
            tracker_system_rate_hz=float(getattr(self.args, "osc_tracker_system_rate_hz", 10.0)),
            tracker_systemB_enabled=bool(getattr(self.args, "osc_tracker_systemB_enable", True)),
            tracker_systemB_rate_hz=float(getattr(self.args, "osc_tracker_systemB_rate_hz", 5.0)),
            tracker_systemB_min_delta=float(getattr(self.args, "osc_tracker_systemB_min_delta", 0.01)),
            tracker_systemB_tau_s=float(getattr(self.args, "osc_tracker_systemB_tau_s", 8.0)),
    ...
    def _start_worker(self) -> None:
        ...
        self._cv_worker = CvScoreWorker(
            args=self.args,
            panel_width=int(self.panel_width),
            cv_display_height=int(cv_display_height),
            score_height=int(score_height),
            osc_track_sender=self.osc_track_sender,
        )
        self._cv_worker.moveToThread(self._cv_thread)

        # Start/stop lifecycle
        self._cv_thread.started.connect(self._cv_worker.start)
        self._cv_thread.finished.connect(self._cv_worker.deleteLater)

        # UI -> worker parameter updates
        self.sig_contrast.connect(self._cv_worker.set_contrast)
        self.sig_brightness.connect(self._cv_worker.set_brightness)
        self.sig_gamma.connect(self._cv_worker.set_gamma)
    ...
    def _on_worker_status(self, msg: str) -> None:
        # Keep this lightweight; printing is fine but can be noisy.
        try:
            print(str(msg))
        except Exception:
            pass
    ...
    def _on_worker_frame_ready(self, cv_vis_bgr, overlays_data, score_vis_bgr) -> None:
        # cv panel
        if cv_vis_bgr is not None:
            overlays = []
            for item in overlays_data or []:
                try:
                    text, x, y, rgb, pt = item
                    r, g, b = rgb
                    overlays.append((text, int(x), int(y), QColor(int(r), int(g), int(b)), int(pt)))
                except Exception:
                    continue
            self.cv_widget.display_cv_image(cv_vis_bgr, overlays=overlays)

        # score panel
        if score_vis_bgr is not None:
            self.score_widget.display_cv_image(score_vis_bgr)
    ...
    # NOTE: CV image adjustment + detection now run inside CvScoreWorker.
    ...
    # NOTE: update_frame() is intentionally no longer used; CvScoreWorker drives the visual updates.
    ...
    def _maybe_send_cv_tracks(self, detections: list, frame_w: int, frame_h: int):
        ...
        # Send fixed A..F slots (but skip D and F to reduce Max load)
        for i in range(track_n):
            letter = self._track_letters[i]
            if letter == "D" or letter == "F":
                continue
    ...
    def closeEvent(self, event):
        ...
        try:
            if self._cv_thread is not None:
                ...
                self._cv_thread.wait(1500)
        except Exception:
            pass
        try:
            if self.osc_track_sender is not None:
                self.osc_track_sender.stop()
        except Exception:
            pass
        event.accept()
...
def main():
    ...
    parser.add_argument("--camera-rotate", type=int, default=180,
                       help="Rotate camera image clockwise by degrees: 0|90|180|270 (default: 180)")
    parser.add_argument("--camera-flip", type=str, default="none",
                       help="Flip camera image: none|h|v|hv (default: none)")
    parser.add_argument("--camera-role", type=str, default="score",
                       help="Camera role name for auto-identification (default: score)")
    parser.add_argument("--camera-auto", dest="camera_auto", action="store_true", default=True,
                       help="(Default: enabled) Auto-pick camera index by role fingerprint when available")
    parser.add_argument("--camera-auto-disable", dest="camera_auto", action="store_false",
                       help="Disable auto role matching; use --camera-index directly")
    parser.add_argument("--camera-roles-file", type=str, default=str(DEFAULT_ROLES_PATH),
                       help="Path to camera role fingerprints JSON (default: python-light-engine/camera_roles.json)")
    parser.add_argument("--camera-calibrate-role", type=str, default="",
                       help="If set (e.g. 'score' or 'motor'), capture a fingerprint from --camera-index and save it, then exit")
    ...
    # CV: ignore humans (suppresses detections that fall on people)
    parser.add_argument("--cv-ignore-people", dest="cv_ignore_people", action="store_true", default=True,
                       help="(Default: enabled) Use a lightweight person detector to suppress detections on humans")
    parser.add_argument("--cv-allow-people", dest="cv_ignore_people", action="store_false",
                       help="Disable person suppression (allow detections on humans)")
    parser.add_argument("--cv-people-rate-hz", type=float, default=2.0,
    ...
    parser.add_argument("--osc-tracker-systemB-enable", action="store_true", default=True,
                        help="(Default: enabled) Also send /system/stateB (slow-evolving state stream for Max)")
    parser.add_argument("--osc-tracker-systemB-disable", action="store_true",
                        help="Disable /system/stateB (overrides --osc-tracker-systemB-enable)")
    parser.add_argument("--osc-tracker-systemB-rate-hz", type=float, default=5.0,
                        help="Rate limit for /system/stateB in Hz (<=0 disables; default: 5.0)")
    parser.add_argument("--osc-tracker-systemB-min-delta", type=float, default=0.01,
                        help="Min delta to send /system/stateB updates (default: 0.01)")
    parser.add_argument("--osc-tracker-systemB-tau-s", type=float, default=8.0,
                        help="Smoothing time-constant in seconds for /system/stateB (larger = slower; default: 8.0)")
    ...
    try:
        roles_path = Path(str(getattr(args, "camera_roles_file", str(DEFAULT_ROLES_PATH))))
    except Exception:
        roles_path = DEFAULT_ROLES_PATH

    calibrate_role = str(getattr(args, "camera_calibrate_role", "") or "").strip().lower()
    if calibrate_role:
        cap = None
        try:
            cap = cv2.VideoCapture(int(args.camera_index), cv2.CAP_ANY)
            if cap is None or not cap.isOpened():
                print(f"[CAM] calibrate failed: cannot open index={int(args.camera_index)}")
                return
            ok, frame = cap.read()
            if not ok or frame is None:
                print(f"[CAM] calibrate failed: cannot read frame index={int(args.camera_index)}")
                return
            h = dhash_hex_from_bgr(frame)
            save_role(calibrate_role, h, index_hint=int(args.camera_index), path=roles_path)
            print(f"[CAM] calibrated role='{calibrate_role}' index={int(args.camera_index)} dhash={h} file={str(roles_path)}")
        finally:
            try:
                if cap is not None:
                    cap.release()
    ...
    if bool(getattr(args, "camera_auto", True)):
        role = str(getattr(args, "camera_role", "score") or "score").strip().lower()
        idx = probe_best_index_for_role(role, max_index=8, path=roles_path)
        if idx is not None:
            args.camera_index = int(idx)
            print(f"[CAM] auto role='{role}' -> index={int(idx)} (file={str(roles_path)})")
        else:
            print(f"[CAM] auto role='{role}' has no fingerprint; using index={int(args.camera_index)}. Calibrate with: --camera-calibrate-role {role} --camera-index <idx>")
    ...
    # /system/stateB enable/disable (separate from /system/state)
    if bool(getattr(args, "osc_tracker_systemB_disable", False)):
        args.osc_tracker_systemB_enable = False
    else:
        args.osc_tracker_systemB_enable = bool(getattr(args, "osc_tracker_systemB_enable", True))
